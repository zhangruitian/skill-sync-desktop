...
- **App settings** (`AppSettings`): Singleton backed by `UserDefaults`. Publishes changes via `@Published`. Agents list and hub profiles are JSON-encoded; ignore patterns are string arrays. Default agents: claude (`~/.claude/skills`), codex (`~/.codex/skills`), agents (`~/.agents/skills`).
